...
export function useStateLobbyChat(stateId: string) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      channel = supabase
        .channel(`room-${roomRow.id}-${Date.now()}`)
        .on(
          "postgres_changes",
      ...
      if (uid) {
        ...
        supabase.from("room_visits").upsert(
          { user_id: uid, room_id: roomRow.id, state: stateId, room_type: "general", last_visited: new Date().toISOString() },
